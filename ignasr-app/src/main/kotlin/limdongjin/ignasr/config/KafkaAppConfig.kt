...
open class KafkaAppConfig(
    ...
    fun kafkaAdmin(): KafkaAdmin? {
        ...
        if (securityProtocol == SecurityProtocol.SASL_PLAINTEXT.name || securityProtocol == SecurityProtocol.SASL_SSL.name) {
            configs[CommonClientConfigs.SECURITY_PROTOCOL_CONFIG] = securityProtocol
            configs[SaslConfigs.SASL_MECHANISM] = saslMechanism
    ...
    fun buildSenderOptions(): SenderOptions<String, ByteArray> {
        ...
        if(securityProtocol.equals("SASL_PLAINTEXT") || securityProtocol.equals("SASL_SSL")){
            producerProps["security.protocol"] = securityProtocol
            producerProps["sasl.mechanism"] = saslMechanism
